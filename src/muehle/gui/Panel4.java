...
public class Panel4 extends JPanel implements MouseListener {
	...
	private String resourceText[] = { "Analysiere Bild...",
			"Bild analysiert!  -  Warte auf Eingabe" };
		...
		for (Position p : Position.getAllPositions()) {
			...
			if (board.getColor(p) == muehle.model.Board.StoneColor.BLACK)
				button[Frame.getGuiPosition(p)]
						.setBackground(Linker.opponentColor);
			else if (board.getColor(p) == muehle.model.Board.StoneColor.WHITE)
				button[Frame.getGuiPosition(p)]
						.setBackground(Linker.humanColor);
			else
	...
	private void drawField(Graphics g) {
		...
		g.drawString(Linker.name1, (int) (1 * a) - (s / 2) + s + 5,
				(int) (8 * b) + s - 3);
		g.drawString(Linker.name2, (int) (4 * a) - (s / 2) + s + 5,
				(int) (8 * b) + s - 3);

		if (Linker.usewebcam1 || Linker.usewebcam2) {
			g.setColor(resourceColor[bildMode]);
			g.drawString(resourceText[bildMode], (int) (0 * a) - (s / 2) + s
					+ 5, (int) (8 * b) - 9);
		}
