	...
	public void thread() {
		if (isButton1) {
			...
			if (Linker.frame.panel2.mouseClicked()) {
				...
				Linker.frame.panel2.setStarterPosition(Linker.frame.panel2
						.getMousePosition());
				isButton1 = false;
				starterSet = true;
		...
		if (isButton3) {
			...
			if (Linker.frame.panel2.mouseClicked()) {
				button3.setBackground(Color.orange);
				Linker.frame.setCursor(Cursor.DEFAULT_CURSOR);
				Linker.frame.panel2.setFieldPosition(Linker.frame.panel2
						.getMousePosition());
				isButton3 = false;
				Linker.frame.panel2.setMouseClick(false);
			}
		...
		try {
			Linker.frame.panel3.button3.setText("Punkt hinzufügen ["
					+ Linker.frame.panel2.getPositionIndex() + "|24]");
		} catch (Exception e) {
		}
		if (isButton4) {
